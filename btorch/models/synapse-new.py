...
class HeterSynapseDualPSC(BasePSC):
    def __init__(
        ...
        **kwargs,
        ...
        # ================== DEBUG START ==================
        print(f"\n[HeterSynapseDualPSC Debug]")
        print(f"  Expected Output Dim: {n_neuron * n_receptor} (N={n_neuron} * R={n_receptor})")
        
        # 检查 Linear 层内部的矩阵形状
        # 假设 SparseConn 将权重存储在 .weight, .matrix 或 .magnitude 中
        # 我们尝试打印它的一般属性
        if hasattr(linear, 'weight'):
            print(f"  Linear.weight shape: {linear.weight.shape}")
        elif hasattr(linear, 'matrix'):
            print(f"  Linear.matrix shape: {linear.matrix.shape}")
        elif hasattr(linear, 'magnitude'): # btorch.linear.SparseConn 通常使用 magnitude
            print(f"  Linear.magnitude shape: {linear.magnitude.shape}")
        # ================== DEBUG END ==================
    ...
    def single_step_forward(self, z: torch.Tensor):
        # 确保 z 的维度正确
        if z.dim() == 1:  # 无 batch 维度
            z = z.unsqueeze(0)  # [N] -> [1, N]
        
        # 如果 z 的维度是 [B, N*R]，直接使用
        # 如果 z 的维度是 [B, N]，需要扩展为 [B, N*R]
        if z.size(-1) == self.size:  # 需要扩展
            z = z.unsqueeze(-1).expand(-1, -1, self.n_receptor).reshape(z.size(0), -1)
        ...
        # 确保 base_psc 输出长度正确
        assert psc.shape[-1] == self.size * self.n_receptor, \
            f"psc last dim {psc.shape[-1]} ≠ N*R {self.size*self.n_receptor}"
