...
def _align_numba(
    ...
    if values.ndim == 1:
        ...
        # Arbitrary leading dimensions: iterate over all leading index tuples
        for leading_idx in np.ndindex(values.shape[:-1]):
            for i in range(n_target):
                key = (int(q_target[i]), int(r_target[i]))
                if key in coord_to_idx:
                    result[leading_idx + (i,)] = values[
                        leading_idx + (coord_to_idx[key],)
                    ]
