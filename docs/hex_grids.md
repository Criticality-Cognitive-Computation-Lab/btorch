...
These are the same as zigzag coordinates. `coord_format="flywire"` is an **alias** for `"zigzag"` — both parse the same input format. The name `"flywire"` exists for readability when working with FlyWire saved-page data.

**Important**: Zigzag/FlyWire coordinates are purely for display/layout. They create the classic hex zigzag where adjacent cells alternate between two columns. The reverse conversion is exact because the rounding rule is consistent.
...
- `coord_format="zigzag"` (or `"flywire"`, alias) for `(p,q)` as zigzag display coordinates
- `coord_format="pixel"` for direct pixel plotting

Additional options:

- `orientation` for axial/zigzag projection (`"pointy"` or `"flat"`)
- `layout` for pixel projection override — use `"flywire"` for website-matching CSS layout, or `None` for default
- `rotation_deg` for optional global display rotation in flywire layout mode
...
fig = heatmap(
    ...
    layout="flywire",    # website-matching pixel positions
